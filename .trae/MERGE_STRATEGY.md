...
| 日期 | 教训 | 改进措施 |
|------|------|----------|
| 2026-06-29 | Edit 工具返回"成功"但内容未真正进入工作区/commit；`findstr` 误报 commit 内容缺失导致误判 | **禁止**依据 Edit 返回提示判断写入成功；**禁止**用 `findstr` 验证含中文内容。提交前**只能**用 `git diff` 验证工作区，提交后**只能**用 `git show` 配合 `Select-String` 验证 commit |
| 2026-06-29 | Read 工具显示版本号为 1.4，实际磁盘仍为 1.3（显示与磁盘不一致） | **禁止**依据 Read 工具显示判断文件真实内容；关键字段**只能**用 `git diff`（提交前）或 `git show`（提交后）确认，二者直接读磁盘/对象库，绕开工具缓存层 |
